    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      {wallets.length === 0 ? (
        <div className="rounded-2xl p-16 text-center" style={{ border: '2px dashed var(--border)', background: 'var(--surface)' }}>
          ...
          <p className="font-bold mb-1" style={{ color: 'var(--text-primary)' }}>Belum ada dompet</p>
          ...
          <button onClick={openNew} className="btn-primary mx-auto px-5 py-2.5 text-sm">
            <Plus size={14} /> Tambah Dompet Pertama
          </button>
        </div>
      ) : (
        <>
      ...
      <div>
        ...
        {transfers.length === 0 ? (
          <div className="rounded-2xl p-16 text-center" style={{ border: '2px dashed var(--border)', background: 'var(--surface)' }}>
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'var(--accent-bg)' }}>
              <ArrowRightLeft size={28} style={{ color: 'var(--accent)' }} />
            </div>
            <p className="font-bold mb-5" style={{ color: 'var(--text-primary)' }}>Belum ada transfer antar dompet.</p>
            <button onClick={openNewTransfer} disabled={wallets.filter(w => w.isActive).length < 2} className="btn-primary mx-auto px-5 py-2.5 text-sm">
              <Plus size={14} /> Transfer Antar Dompet Pertama
            </button>
          </div>
        ) : (
          <>
